...
void GLViewNewModule::updateWorld()
{
   ...
   // When starting 3D sounds, update position to camera position all the time.
   if (!this->smgr->getSound3D().empty()) {
	   cout << "Update ListenerPosition!!!" << endl;
	   cout << "Camera Current Position: " << this->cam->getPosition() << endl;
	   this->smgr->getSoundEngine()->setListenerPosition(
		   this->smgr->convertFromVector(this->cam->getPosition()),
		   this->smgr->convertFromVector(this->cam->getLookDirection()),
		   irrklang::vec3df(0, 0, 0),
		   this->smgr->convertFromVector(this->cam->getNormalDirection())
	   );
   }
}
...
void GLViewNewModule::onKeyDown( const SDL_KeyboardEvent& key )
{
   ...
   // Press 's' to stop a 2D sound.
   if (key.keysym.sym == SDLK_s) {
	   cout << "Stopping playing 2D music!!!" << endl;
	   if (!this->smgr->getSound2D().empty() && !this->smgr->getSound2D().at(0)->isFinished()) {
		   this->smgr->stopSound2D();
		   cout << "Stopped!!!" << endl;
	   }
   }

   // Press 'b' to begin 3D sound.
   // Press multiple times will add the same music every time...Not suggested..
   // ...but you can press 'e' to stop one 3D sound..
   // Press 'a' to stop all sounds...
   if (key.keysym.sym == SDLK_b) {
	   cout << "Start playing 3D music!!!" << endl;
	   Vector cur_cam_pos = this->cam->getPosition();
	   cout << "Camera current position is:\n" << cur_cam_pos << endl;
	   this->smgr->play3D("../../../shared/mm/sounds/sound2.wav", Vector(15, 15, 10), true, false, true);
	   cout << this->smgr->getSound3D().size() << endl;
	   this->smgr->getSound3D().back()->setVolume(0.9f);
	   this->smgr->getSound3D().back()->setMinDistance(10.0f);
   }

   // Press 'e' to stop a 3D sound.
   if (key.keysym.sym == SDLK_e) {
	   cout << "Stopping playing 3D music!!!" << endl;
	   if (!this->smgr->getSound3D().empty() && !this->smgr->getSound3D().at(0)->isFinished()) {
		   this->smgr->stopSound3D();
		   cout << "Stopped!!!" << endl;
	   }
   }
   ...
   if (key.keysym.sym == SDLK_a) {
	   cout << "Stopping all the playing sounds!!!" << endl;
	   if (this->smgr->getSoundEngine() != nullptr) {
		   this->smgr->getSoundEngine()->stopAllSounds();
		   cout << "Stopped all sounds..." << endl;
		   cout << "Will be quiet now..." << endl;
	   }
   }
}
